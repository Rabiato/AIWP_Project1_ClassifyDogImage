...
def classify_images(images_dir, results_dic, model):
    ...
    classifier_labels = []
    # adding classifier result to label list
    for filename in filenames:
        classifier_labels.append(classifier(images_dir + filename, model))
    # formating classifier label list to lower case and strip whitespace
    for i in range(0, len(classifier_labels), 1):
        classifier_labels[i] = classifier_labels[i].lower().strip()
    # append classifier labels to result_dic and check for matches
    for idx in range (0, len(filenames), 1):
        # adds labels of classifier to dictionary
